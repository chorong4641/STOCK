...
def suggestion(request):
    if request.method == 'POST':
        try:
            ...
            if request_data['period'] == '3month':
                start_date = (datetime.datetime.strptime('2020-12-31','%Y-%m-%d') + relativedelta(months=-3)).strftime("%Y-%m-%d")
            elif request_data['period'] == '6month':
                start_date = (datetime.datetime.strptime('2020-12-31','%Y-%m-%d') + relativedelta(months=-6)).strftime("%Y-%m-%d")
            elif request_data['period'] == '9month':
                start_date = (datetime.datetime.strptime('2020-12-31','%Y-%m-%d') + relativedelta(months=-9)).strftime("%Y-%m-%d")
            else :
                start_date = (datetime.datetime.strptime('2020-12-31','%Y-%m-%d') + relativedelta(years=-1)).strftime("%Y-%m-%d")
            
            end_date = '2021-12-31'
            df_marcap = pd.DataFrame(list(Krx.objects.filter(date__range=[start_date,end_date]).values()))
            ...
            cols = pd.DataFrame(marcap_sector.loc['2020-12-30'].sort_values(ascending=False)).head(10).index
            graph = marcap_sector.loc[(datetime.datetime.strptime(start_date,'%Y-%m-%d') + datetime.timedelta(1)).strftime("%Y-%m-%d"):, cols].to_json()
            
            top_sector = marcap_sector.loc['2020-12-30'].sort_values(ascending=False)[:10]
            top_sector = top_sector.to_dict()   # 1등섹터
            
            sug = []
            # 1등섹터에서 시가총액1위
            for s in top_sector.keys():
                temp = df[(df['date'] == '2020-12-30') & (df['Sector'] == s)]
