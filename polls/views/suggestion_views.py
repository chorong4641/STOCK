    ...
    if request.method == 'POST':
        try:
            ...
            if request_data['period'] == '3month':
                start_date = (datetime.datetime.strptime('2021-11-10','%Y-%m-%d') + relativedelta(months=-3)).strftime("%Y-%m-%d")
            elif request_data['period'] == '6month':
                start_date = (datetime.datetime.strptime('2021-11-10','%Y-%m-%d') + relativedelta(months=-6)).strftime("%Y-%m-%d")
            elif request_data['period'] == '9month':
                start_date = (datetime.datetime.strptime('2021-11-10','%Y-%m-%d') + relativedelta(months=-9)).strftime("%Y-%m-%d")
            else :
                start_date = (datetime.datetime.strptime('2021-11-10','%Y-%m-%d') + relativedelta(years=-1)).strftime("%Y-%m-%d")
